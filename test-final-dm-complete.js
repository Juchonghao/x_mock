 ...
 */
async function testFinalDMComplete() {
  console.log('🚀 开始最终版私信发送测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试修复了所有已知问题，包括selector.startsWith错误');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `final-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `final-dm-ready-${username}.png`), 
          fullPage: true 
        });
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是最终版测试，修复了所有已知问题。祝你一切顺利！🤖`;
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `final-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'final-test-complete.png'), 
      fullPage: true 
    });
...
async function openDMFromUserProfileFinal(page, username) {
  try {
    ...
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/final-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/final-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: `./sessions/final-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
...
async function handlePinVerificationFinal(page) {
  try {
    ...
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/final-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/final-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 发送私信消息 - 最终版，修复selector.startsWith错误
 */
async function sendDMMessageFinal(page, message, username) {
  try {
    ...
    // 输入消息 - 完全修复selector.startsWith错误
    try {
      // 确保 inputElement 是一个有效的 DOM 元素
      if (inputElement && typeof inputElement === 'object' && inputElement.constructor.name === 'ElementHandle') {
      ...
      try {
        ...
        console.log('⚠️ 直接设置值也失败:', evalError.message);
      }
    }
...
// 运行测试
testFinalDMComplete().then(results => {
  console.log('\n🏁 最终版测试脚本执行完毕');
