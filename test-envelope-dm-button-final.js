...
/**
 * 最终版测试给特定用户发送私信 - 优化信封图标按钮查找和消息发送
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testEnvelopeDMButtonFinal() {
  console.log('🚀 开始最终版测试信封图标私信按钮');
...
async function sendDMMessage(page, message) {
  try {
    ...
    const sendButtonSelectors = [
      ...
      'button[type="submit"]'  // 有时发送按钮是submit类型
    ];
...
// 运行测试
testEnvelopeDMButtonFinal().then(results => {
  console.log('\n🏁 最终版测试脚本执行完毕');
  process.exit(0);
}).catch(error => {
