 ...
 */
async function testFixUserClick() {
  console.log('🚀 开始修复用户点击功能的聊天流程测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试重点修复搜索结果中点击用户的功能');
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'fix-user-click-initial.png'), 
      fullPage: true 
    });
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        // 点击"New message"按钮
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChatUserClickFixed(page);
        ...
        const newChatPasscodeHandled = await handlePasscodeInNewChatUserClickFixed(page);
        ...
        const userFound = await searchUserInNewChatUserClickFixed(page, username);
        ...
        const chatStarted = await startChatWithUserUserClickFixed(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationUserClickFixed(page);
        ...
        const chatPasscodeHandled = await handlePasscodeInChatUserClickFixed(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `fix-user-click-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessageUserClickFixed(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过修复用户点击功能的聊天流程发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageUserClickFixed(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `fix-user-click-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'fix-user-click-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 打开新聊天对话框 - 用户点击修复版本
 */
async function openNewChatUserClickFixed(page) {
  try {
    console.log('🔍 查找新消息按钮...');
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/fix-user-click-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/fix-user-click-after-new-chat.png', 
      fullPage: true 
    });
...
/**
 * 在新聊天界面处理passcode验证 - 用户点击修复版本
 */
async function handlePasscodeInNewChatUserClickFixed(page) {
  try {
...
/**
 * 在新聊天界面中搜索用户 - 用户点击修复版本
 */
async function searchUserInNewChatUserClickFixed(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/fix-user-click-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    for (const selector of searchSelectors) {
      try {
        const element = await page.$(selector, { timeout: 10000 });
        if (element) {
          // 检查这个输入框是否适合搜索用户
          ...
          if (placeholder && (placeholder.toLowerCase().includes('search') || 
                             ...
                             placeholder.toLowerCase().includes('用户'))) {
            searchInput = element;
            foundSelector = selector;
            console.log(`✅ 找到用户搜索输入框: ${selector} (placeholder: "${placeholder}")`);
            break;
          }
          ...
          if (ariaLabel && (ariaLabel.toLowerCase().includes('search') || 
                           ...
                           ariaLabel.toLowerCase().includes('用户'))) {
            searchInput = element;
            foundSelector = selector;
            console.log(`✅ 找到用户搜索输入框: ${selector} (aria-label: "${ariaLabel}")`);
            break;
          }
        }
      } catch (e) {
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/fix-user-click-search-${username}-after.png`, 
      fullPage: true 
    });
 ...
 * 重点修复搜索结果中点击用户的功能
 */
async function startChatWithUserUserClickFixed(page, username) {
  try {
    ...
    // 拍摄搜索结果截图，帮助调试
    await page.screenshot({ 
      path: `./sessions/fix-user-click-search-results-${username}.png`, 
      fullPage: true 
    });
    
    // 方法1: 尝试直接点击包含用户名的按钮
    console.log('🔍 尝试直接点击包含用户名的元素...');
    const userSelectors = [
      `a[href="/${username}"]`,
    ...
    for (const selector of userSelectors) {
      try {
        // 使用 page.$x 进行XPath搜索，更精确地定位用户元素
        const elements = await page.$x(`//div[contains(@role, "button") and (contains(text(), "${username}") or contains(text(), "@${username}"))]`);
        if (elements.length > 0) {
          userElement = elements[0];
          console.log(`✅ 通过XPath找到用户 @${username} 元素`);
          break;
        }
      } catch (e) {
        console.log(`⚠️ XPath搜索失败:`, e.message);
        continue;
      }
      
      // 如果XPath方法没找到，尝试CSS选择器
      if (!userElement) {
        try {
          const elements = await page.$$(selector);
          for (const element of elements) {
            try {
              ...
              if (textContent && (textContent.includes(username) || textContent.includes(`@${username}`))) {
                userElement = element;
                console.log(`✅ 通过CSS选择器找到用户 @${username} 元素: "${textContent}"`);
                break;
              }
            } catch (evalError) {
              continue;
    ...
    // 方法2: 如果上述方法都没找到，尝试更通用的方法
    if (!userElement) {
      console.log('⚠️ 未找到特定用户元素，尝试更通用的搜索...');
      
      // 查找所有按钮元素
      const allButtons = await page.$$('div[role="button"], a[role="link"]');
      ...
      for (const button of allButtons) {
        try {
          ...
          if (textContent && (textContent.includes(username) || textContent.includes(`@${username}`))) {
            ...
            // 如果按钮包含用户相关信息，认为是正确的用户按钮
            if (ariaLabel && (ariaLabel.includes('user') || ariaLabel.includes('User') || 
                              ariaLabel.includes('profile') || ariaLabel.includes('Profile'))) {
              userElement = button;
              console.log(`✅ 通过用户相关信息找到用户元素: "${textContent}", aria-label: "${ariaLabel}"`);
              break;
            } else if (dataTestId && (dataTestId.includes('user') || dataTestId.includes('User'))) {
              userElement = button;
              console.log(`✅ 通过data-testid找到用户元素: "${textContent}", data-testid: "${dataTestId}"`);
              break;
            } else {
              // 如果没有明确的用户相关信息，但包含用户名，也认为是可能的用户元素
              userElement = button;
              console.log(`✅ 通过文本内容找到可能的用户元素: "${textContent}"`);
              break;
            }
          }
        } catch (e) {
    ...
    // 方法3: 最后尝试使用更通用的搜索方法
    if (!userElement) {
      console.log('⚠️ 尝试最后的通用搜索方法...');
      try {
        // 使用evaluate方法查找包含用户名的元素
        const elementHandle = await page.evaluateHandle((user) => {
          // 查找所有可能的用户元素
          const possibleElements = document.querySelectorAll('div[role="button"], a[role="link"]');
        ...
        if (elementHandle && elementHandle.asElement()) {
          ...
          console.log(`✅ 通过evaluate方法找到用户 @${username} 元素`);
        }
      } catch (evalError) {
    ...
    // 等待元素可点击
    try {
      await userElement.waitForElementState('visible');
    } catch (e) {
      console.log('⚠️ 元素可能不可见，继续尝试点击');
    }
    ...
    // 拍摄开始聊天后截图
    await page.screenshot({ 
      path: `./sessions/fix-user-click-start-chat-${username}.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 用户点击修复版本
 */
async function handlePinVerificationUserClickFixed(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/fix-user-click-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/fix-user-click-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 在聊天中处理passcode验证 - 用户点击修复版本
 */
async function handlePasscodeInChatUserClickFixed(page) {
  try {
...
/**
 * 检查是否有"unencrypted message"提示 - 用户点击修复版本
 */
async function checkUnencryptedMessageUserClickFixed(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
 ...
 * 发送私信消息 - 用户点击修复版本
 */
async function sendDMMessageUserClickFixed(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testFixUserClick().then(results => {
  console.log('\n🏁 修复用户点击功能的聊天流程测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 修复用户点击功能的聊天流程测试脚本执行失败:', error);
  process.exit(1);
});