...
/**
 * 最终版：处理passcode验证并直接在unencrypted message输入框发送私信的测试
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testFinalPasscodeUnencrypted() {
  console.log('🚀 开始最终版私信发送测试（处理passcode + unencrypted message输入框）');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试将处理passcode验证并直接在unencrypted message输入框发送消息');
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `final-passcode-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        const dmOpened = await openDMFromUserProfileFinal(page, username);
        ...
        // 重要：检查并处理passcode验证 - 这是关键步骤
        console.log('🔍 检查是否需要passcode验证...');
        const passcodeHandled = await handlePasscodeIfPresentFinal(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是最终版测试，处理了passcode验证并直接在unencrypted message输入框发送。祝你一切顺利！🤖`;
 ...
 * 从用户资料页面打开私信 - 最终版
 */
async function openDMFromUserProfileFinal(page, username) {
  try {
    console.log(`🔍 在 @${username} 的页面上查找私信按钮...`);
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/final-passcode-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/final-passcode-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: `./sessions/final-passcode-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
...
/**
 * 检查并处理passcode验证 - 最终版（这是关键部分）
 */
async function handlePasscodeIfPresentFinal(page) {
  try {
    console.log('🔍 检查是否需要passcode验证...');
...
/**
 * 检查是否有"unencrypted message"提示 - 最终版
 */
...
/**
 * 发送私信消息 - 最终版，处理passcode并直接在unencrypted message输入框发送
 */
...
// 运行测试
testFinalPasscodeUnencrypted().then(results => {
  console.log('\n🏁 最终版测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 最终版测试脚本执行失败:', error);
  process.exit(1);
});