...
/**
 * 优先处理passcode验证的聊天流程测试
 * 进入chat后首先处理passcode问题，然后搜索用户并发送私信
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testPasscodeFirstChat() {
  console.log('🚀 开始优先处理passcode验证的聊天流程测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试先进入chat页面，优先处理passcode问题，再发送私信');
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'passcode-first-initial.png'), 
      fullPage: true 
    });
    
    // 优先检查并处理可能存在的passcode问题
    console.log('🔐 优先检查并处理可能存在的passcode验证...');
    const passcodeHandled = await handlePasscodeOnChatPage(page);
    
    if (passcodeHandled) {
      console.log('✅ 聊天页面passcode验证处理完成');
      await new Promise(resolve => setTimeout(resolve, 3000));
    } else {
      console.log('ℹ️ 聊天页面未检测到passcode验证需求');
    }
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        // 点击"New message"或"New chat"按钮
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChatPasscodeFirst(page);
        ...
        // 再次检查并处理passcode问题（在新聊天界面）
        console.log('🔐 检查新聊天界面的passcode验证...');
        const newChatPasscodeHandled = await handlePasscodeOnNewChatPage(page);
        ...
        const userFound = await searchUserInNewChatPasscodeFirst(page, username);
        ...
        const chatStarted = await startChatWithUserPasscodeFirst(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationPasscodeFirst(page);
        ...
        // 在聊天界面再次检查并处理passcode问题
        console.log('🔐 聊天界面检查passcode验证...');
        const chatPasscodeHandled = await handlePasscodeInChat(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `passcode-first-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessagePasscodeFirst(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过优先处理passcode验证的聊天流程发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessagePasscodeFirst(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `passcode-first-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'passcode-first-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 在聊天页面处理passcode验证
 */
async function handlePasscodeOnChatPage(page) {
  try {
    console.log('🔍 检查聊天页面是否需要passcode验证...');
    ...
    for (const selector of passcodeSelectors) {
      try {
        const passcodeInput = await page.$(selector, { timeout: 5000 });
        if (passcodeInput) {
          console.log(`🔐 检测到passcode输入框: ${selector}，开始处理...`);
          ...
          for (const confirmSelector of confirmSelectors) {
            try {
              confirmButton = await page.$(confirmSelector, { timeout: 5000 });
              if (confirmButton) {
    ...
    console.error('❌ 处理聊天页面passcode验证时出错:', error.message);
    return false;
  }
}

/**
 * 打开新聊天对话框 - passcode优先版本
 */
async function openNewChatPasscodeFirst(page) {
  try {
    console.log('🔍 查找新消息按钮...');
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/passcode-first-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/passcode-first-after-new-chat.png', 
      fullPage: true 
    });
 ...
 * 在新聊天页面处理passcode验证
 */
async function handlePasscodeOnNewChatPage(page) {
  try {
    console.log('🔍 检查新聊天页面是否需要passcode验证...');
    ...
    for (const selector of passcodeSelectors) {
      try {
        const passcodeInput = await page.$(selector, { timeout: 5000 });
        if (passcodeInput) {
          console.log(`🔐 检测到新聊天页面passcode输入框: ${selector}，开始处理...`);
    ...
    console.log('ℹ️ 新聊天页面未检测到passcode验证需求');
    return false;
    
  } catch (error) {
    console.error('❌ 处理新聊天页面passcode验证时出错:', error.message);
    return false;
  }
}

/**
 * 在新聊天界面中搜索用户 - passcode优先版本
 */
async function searchUserInNewChatPasscodeFirst(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/passcode-first-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/passcode-first-search-${username}-after.png`, 
      fullPage: true 
    });
...
/**
 * 点击用户开始聊天 - passcode优先版本
 */
async function startChatWithUserPasscodeFirst(page, username) {
  try {
    ...
    // 拍摄开始聊天后截图
    await page.screenshot({ 
      path: `./sessions/passcode-first-start-chat-${username}.png`, 
      fullPage: true 
    });
...
/**
 * 处理PIN验证 - passcode优先版本
 */
async function handlePinVerificationPasscodeFirst(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/passcode-first-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/passcode-first-after-pin-submit.png', 
      fullPage: true 
    });
 ...
 * 在聊天中处理passcode验证
 */
async function handlePasscodeInChat(page) {
  try {
    ...
    for (const selector of passcodeSelectors) {
      try {
        const passcodeInput = await page.$(selector, { timeout: 5000 });
        if (passcodeInput) {
          console.log(`🔐 检测到聊天界面passcode输入框: ${selector}，开始处理...`);
...
/**
 * 检查是否有"unencrypted message"提示 - passcode优先版本
 */
async function checkUnencryptedMessagePasscodeFirst(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
 ...
 * 发送私信消息 - passcode优先版本
 */
async function sendDMMessagePasscodeFirst(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testPasscodeFirstChat().then(results => {
  console.log('\n🏁 优先处理passcode验证的聊天流程测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 优先处理passcode验证的聊天流程测试脚本执行失败:', error);
  process.exit(1);
});