 ...
 * 优化处理对话框已打开的情况
 */
async function testSpecificUsersDMImproved() {
  console.log('🚀 开始改进版测试给特定用户发送私信');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await dmService.screenshot(`improved-user-${username}-page.png`);
        ...
        // 等待对话框完全加载
        await new Promise(resolve => setTimeout(resolve, 3000));
        await dmService.screenshot(`improved-dm-opened-${username}.png`);
        
        // 检查是否已经可以看到消息输入框（"unencrypted message"状态）
        ...
        if (!isReady) {
          ...
          await dmService.screenshot(`improved-check-after-delay-${username}.png`);
        }
        ...
        if (messageSent) {
          ...
          await dmService.screenshot(`improved-message-sent-${username}.png`);
    ...
    // 拍摄最终状态截图
    await dmService.screenshot('improved-test-complete.png');
...
async function checkIfDMReady(dmService) {
  try {
    ...
    // 检查是否有"unencrypted message"相关元素
    ...
    for (const selector of unencryptedSelectors) {
      try {
        // 使用 evaluate 方法查找包含特定文本的元素
        const hasUnencrypted = await dmService.page.evaluate(() => {
          const allElements = document.querySelectorAll('*');
          for (let el of allElements) {
            if (el.textContent && 
                (el.textContent.includes('unencrypted') || 
        ...
        if (hasUnencrypted) {
          console.log('✅ 检测到 "unencrypted message" 相关元素，表示可以发送');
          return true;
        }
      } catch (e) {
        continue;
...
// 运行测试
testSpecificUsersDMImproved().then(results => {
  console.log('\n🏁 改进版测试脚本执行完毕');
  process.exit(0);
}).catch(error => {
  console.error('❌ 改进版测试脚本执行失败:', error);
  process.exit(1);
});