...
/**
 * 最终修复passcode验证的聊天流程测试
 * 解决selector.startsWith错误，正确处理passcode输入框
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testFinalPasscodeFix() {
  console.log('🚀 开始最终修复passcode验证的聊天流程测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试解决selector错误，正确处理passcode验证');
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'final-passcode-initial.png'), 
      fullPage: true 
    });
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        // 点击"New message"按钮
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChatFinalFix(page);
        ...
        // 处理新聊天界面的passcode - 最终修复版本
        console.log('🔐 最终修复版本检测新聊天界面的passcode验证...');
        ...
        const userFound = await searchUserInNewChatFinalFix(page, username);
        ...
        const chatStarted = await startChatWithUserFinalFix(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationFinalFix(page);
        ...
        // 在聊天界面处理passcode问题 - 最终修复版本
        console.log('🔐 聊天界面最终修复版本检测passcode验证...');
        const chatPasscodeHandled = await handlePasscodeInChatFinalFix(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `final-passcode-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessageFinalFix(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过最终修复passcode验证的聊天流程发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageFinalFix(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `final-passcode-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'final-passcode-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 打开新聊天对话框 - 最终修复版本
 */
async function openNewChatFinalFix(page) {
  try {
    console.log('🔍 查找新消息按钮...');
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/final-passcode-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/final-passcode-after-new-chat.png', 
      fullPage: true 
    });
 ...
 * 解决selector.startsWith错误
 */
async function handlePasscodeInNewChatFinal(page) {
  try {
    console.log('🔍 最终修复版本检测新聊天界面是否需要passcode验证...');
    
    // 等待页面加载
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    console.log('🔍 使用最终修复方法查找passcode输入框...');
 ...
 * 在新聊天界面中搜索用户 - 最终修复版本
 */
async function searchUserInNewChatFinalFix(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/final-passcode-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/final-passcode-search-${username}-after.png`, 
      fullPage: true 
    });
 ...
 * 点击用户开始聊天 - 最终修复版本
 */
async function startChatWithUserFinalFix(page, username) {
  try {
    ...
    // 拍摄开始聊天后截图
    await page.screenshot({ 
      path: `./sessions/final-passcode-start-chat-${username}.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 最终修复版本
 */
async function handlePinVerificationFinalFix(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/final-passcode-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/final-passcode-after-pin-submit.png', 
      fullPage: true 
    });
 ...
 * 解决selector.startsWith错误
 */
async function handlePasscodeInChatFinalFix(page) {
  try {
    console.log('🔍 最终修复版本检测聊天界面是否需要passcode验证...');
    
    // 等待页面加载
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    console.log('🔍 使用最终修复方法查找聊天界面passcode输入框...');
 ...
 */
async function checkUnencryptedMessageFinalFix(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
 ...
 * 发送私信消息 - 最终修复版本
 */
async function sendDMMessageFinalFix(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testFinalPasscodeFix().then(results => {
  console.log('\n🏁 最终修复passcode验证的聊天流程测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 最终修复passcode验证的聊天流程测试脚本执行失败:', error);
  process.exit(1);
});