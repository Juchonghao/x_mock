...
/**
 * 精确处理passcode验证的聊天流程测试
 * 使用更精确的方法检测和处理passcode输入框
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testPrecisePasscodeFix() {
  console.log('🚀 开始精确处理passcode验证的聊天流程测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试使用精确方法检测和处理passcode验证');
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'precise-passcode-initial.png'), 
      fullPage: true 
    });
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        // 点击"New message"按钮
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChatPreciseFix(page);
        ...
        // 精确检测并处理新聊天界面的passcode
        console.log('🔐 精确检测新聊天界面的passcode验证...');
        const newChatPasscodeHandled = await handlePasscodeInNewChatPrecise(page);
        ...
        const userFound = await searchUserInNewChatPreciseFix(page, username);
        ...
        const chatStarted = await startChatWithUserPreciseFix(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationPreciseFix(page);
        ...
        // 在聊天界面精确处理passcode问题
        console.log('🔐 聊天界面精确检测passcode验证...');
        const chatPasscodeHandled = await handlePasscodeInChatPreciseFix(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `precise-passcode-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessagePreciseFix(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过精确处理passcode验证的聊天流程发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessagePreciseFix(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `precise-passcode-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'precise-passcode-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 打开新聊天对话框 - 精确修复版本
 */
async function openNewChatPreciseFix(page) {
  try {
    console.log('🔍 查找新消息按钮...');
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/precise-passcode-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/precise-passcode-after-new-chat.png', 
      fullPage: true 
    });
...
/**
 * 在新聊天界面精确处理passcode验证 - 精确修复版本
 */
async function handlePasscodeInNewChatPrecise(page) {
  try {
    console.log('🔍 精确检测新聊天界面是否需要passcode验证...');
    
    // 等待页面加载
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    // 使用更精确的方法查找passcode输入框
    console.log('🔍 使用多种方法精确查找passcode输入框...');
    
    // 方法1: 直接查找inputmode="numeric"的输入框（这通常是passcode输入框）
    try {
      const numericInput = await page.$('input[inputmode="numeric"]');
      if (numericInput) {
        ...
        // 输入passcode 0000
        await page.type(numericInput, '0000', { delay: 100 });
 ...
 * 在新聊天界面中搜索用户 - 精确修复版本
 */
async function searchUserInNewChatPreciseFix(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/precise-passcode-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/precise-passcode-search-${username}-after.png`, 
      fullPage: true 
    });
 ...
 * 点击用户开始聊天 - 精确修复版本
 */
async function startChatWithUserPreciseFix(page, username) {
  try {
    ...
    // 拍摄开始聊天后截图
    await page.screenshot({ 
      path: `./sessions/precise-passcode-start-chat-${username}.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 精确修复版本
 */
async function handlePinVerificationPreciseFix(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/precise-passcode-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/precise-passcode-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 在聊天中精确处理passcode验证 - 精确修复版本
 */
async function handlePasscodeInChatPreciseFix(page) {
  try {
    console.log('🔍 精确检测聊天界面是否需要passcode验证...');
    
    // 等待页面加载
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    // 使用更精确的方法查找passcode输入框
    console.log('🔍 使用多种方法精确查找聊天界面passcode输入框...');
    
    // 方法1: 直接查找inputmode="numeric"的输入框
    try {
      const numericInput = await page.$('input[inputmode="numeric"]');
      if (numericInput) {
        console.log('🔐 检测到inputmode="numeric"的输入框，这很可能是passcode输入框，开始处理...');
...
/**
 * 检查是否有"unencrypted message"提示 - 精确修复版本
 */
async function checkUnencryptedMessagePreciseFix(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
 ...
 * 发送私信消息 - 精确修复版本
 */
async function sendDMMessagePreciseFix(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testPrecisePasscodeFix().then(results => {
  console.log('\n🏁 精确处理passcode验证的聊天流程测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 精确处理passcode验证的聊天流程测试脚本执行失败:', error);
  process.exit(1);
});