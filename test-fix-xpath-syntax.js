...
/**
 * 修复XPath语法错误和搜索功能的聊天流程测试
 * 修复XPath语法错误，同时保持passcode处理和搜索功能
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testFixXpathSyntax() {
  console.log('🚀 开始修复XPath语法错误和搜索功能的聊天流程测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试修复XPath语法错误，同时保持passcode处理和搜索功能');
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'fix-xpath-syntax-initial.png'), 
      fullPage: true 
    });
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        // 点击"New message"按钮
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChatXpathFixed(page);
        ...
        const newChatPasscodeHandled = await handlePasscodeInNewChatXpathFixed(page);
        ...
        const userFound = await searchUserInNewChatXpathFixed(page, username);
        ...
        // 点击用户开始聊天 - 修复XPath语法错误
        console.log(`💬 点击用户开始聊天...`);
        const chatStarted = await startChatWithUserXpathFixed(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationXpathFixed(page);
        ...
        const chatPasscodeHandled = await handlePasscodeInChatXpathFixed(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `fix-xpath-syntax-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessageXpathFixed(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过修复XPath语法错误的聊天流程发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageXpathFixed(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `fix-xpath-syntax-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'fix-xpath-syntax-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 打开新聊天对话框 - XPath修复版本
 */
async function openNewChatXpathFixed(page) {
  try {
    console.log('🔍 查找新消息按钮...');
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/fix-xpath-syntax-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/fix-xpath-syntax-after-new-chat.png', 
      fullPage: true 
    });
...
/**
 * 在新聊天界面处理passcode验证 - XPath修复版本
 */
async function handlePasscodeInNewChatXpathFixed(page) {
  try {
...
/**
 * 在新聊天界面中搜索用户 - XPath修复版本
 */
async function searchUserInNewChatXpathFixed(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/fix-xpath-syntax-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/fix-xpath-syntax-search-${username}-after.png`, 
      fullPage: true 
    });
...
/**
 * 点击用户开始聊天 - 修复XPath语法错误版本
 */
async function startChatWithUserXpathFixed(page, username) {
  try {
    ...
    for (const selector of userSelectors) {
      try {
        // 使用page.evaluateHandle来安全地获取元素
        const elementHandle = await page.evaluateHandle((sel, user) => {
          // 查找包含用户名的元素
    ...
    // 如果没找到，尝试更通用的方法 - 修复XPath语法错误
    if (!userElement) {
      console.log('⚠️ 未找到特定用户元素，尝试通用搜索...');
      
      // 修复XPath语法错误：使用CSS选择器而不是XPath
      // 原来的错误语法：'.//div[contains(@role, "button") or contains(@role, "link")]'
      // 现在使用CSS选择器：'div[role="button"], a[role="link"]'
      const possibleUserElements = await page.$$('div[role="button"], a[role="link"]');
    ...
    // 拍摄开始聊天后截图
    await page.screenshot({ 
      path: `./sessions/fix-xpath-syntax-start-chat-${username}.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - XPath修复版本
 */
async function handlePinVerificationXpathFixed(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/fix-xpath-syntax-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/fix-xpath-syntax-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 在聊天中处理passcode验证 - XPath修复版本
 */
async function handlePasscodeInChatXpathFixed(page) {
  try {
...
/**
 * 检查是否有"unencrypted message"提示 - XPath修复版本
 */
async function checkUnencryptedMessageXpathFixed(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
 ...
 * 发送私信消息 - XPath修复版本
 */
async function sendDMMessageXpathFixed(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testFixXpathSyntax().then(results => {
  console.log('\n🏁 修复XPath语法错误和搜索功能的聊天流程测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 修复XPath语法错误和搜索功能的聊天流程测试脚本执行失败:', error);
  process.exit(1);
});