 ...
 */
async function testPasscodeHandledDM() {
  console.log('🚀 开始处理passcode验证的私信发送测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试将处理passcode验证和unencrypted message输入框');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `passcode-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        const dmOpened = await openDMFromUserProfilePasscode(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationPasscode(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `passcode-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有passcode验证 - 这是关键步骤
        console.log('🔍 检查是否需要passcode验证...');
        const passcodeHandled = await handlePasscodeIfPresentPasscode(page);
        ...
        const unencryptedMessageFound = await checkUnencryptedMessagePasscode(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是处理passcode验证的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessagePasscode(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `passcode-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'passcode-test-complete.png'), 
      fullPage: true 
    });
...
/**
 * 从用户资料页面打开私信 - passcode版本
 */
async function openDMFromUserProfilePasscode(page, username) {
  try {
    console.log(`🔍 在 @${username} 的页面上查找私信按钮...`);
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/passcode-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/passcode-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: `./sessions/passcode-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - passcode版本
 */
async function handlePinVerificationPasscode(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/passcode-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/passcode-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 检查并处理passcode验证 - passcode版本
 */
async function handlePasscodeIfPresentPasscode(page) {
  try {
    console.log('🔍 检查是否需要passcode验证...');
    ...
    if (passcodeInput) {
      ...
      for (const selector of confirmSelectors) {
        try {
          ...
          // 尝试使用 page.$eval 作为备选方法
          try {
            confirmButton = await page.$eval(selector, el => el);
            if (confirmButton) {
              console.log(`✅ 通过page.$eval找到确认按钮: ${selector}`);
              break;
            }
          } catch (evalError) {
            continue;
...
/**
 * 检查是否有"unencrypted message"提示 - passcode版本
 */
async function checkUnencryptedMessagePasscode(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
...
/**
 * 发送私信消息 - passcode版本，处理passcode验证
 */
async function sendDMMessagePasscode(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
    ...
    for (const selector of passcodeSelectors) {
      try {
        const passcodeElement = await page.$(selector);
        if (passcodeElement) {
          console.log('⚠️ 检测到passcode框仍然存在，等待处理完成...');
          await new Promise(resolve => setTimeout(resolve, 3000));
          break;
        }
      } catch (e) {
    ...
    // 重新查找消息输入框 - 因为页面可能在passcode处理后发生变化
    const inputSelectors = [
      'div[contenteditable="true"]',
      'textarea',
...
// 运行测试
testPasscodeHandledDM().then(results => {
  console.log('\n🏁 passcode验证处理测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ passcode验证处理测试脚本执行失败:', error);
  process.exit(1);
});