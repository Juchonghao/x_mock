...
/**
 * 直接在unencrypted message输入框中发送私信的测试
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testUnencryptedInputDirect() {
  console.log('🚀 开始直接在unencrypted message输入框发送私信测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试将直接在unencrypted message输入框中输入消息');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `direct-unencrypted-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        const dmOpened = await openDMFromUserProfileDirect(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationDirect(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `direct-unencrypted-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessageDirect(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是直接在unencrypted message输入框发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageDirect(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `direct-unencrypted-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 生成测试报告
    console.log('\n📊 直接在unencrypted message输入框测试结果报告');
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'direct-unencrypted-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 从用户资料页面打开私信 - 直接版本
 */
async function openDMFromUserProfileDirect(page, username) {
  try {
    console.log(`🔍 在 @${username} 的页面上查找私信按钮...`);
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/direct-unencrypted-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/direct-unencrypted-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: `./sessions/direct-unencrypted-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 直接版本
 */
async function handlePinVerificationDirect(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/direct-unencrypted-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/direct-unencrypted-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 检查是否有"unencrypted message"提示 - 直接版本
 */
async function checkUnencryptedMessageDirect(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
...
/**
 * 发送私信消息 - 直接版本，专门针对unencrypted message输入框
 */
async function sendDMMessageDirect(page, message, username) {
  try {
    console.log('📝 准备发送消息到unencrypted message输入框...');
    
    // 等待页面完全加载
    await new Promise(resolve => setTimeout(resolve, 5000)); // 增加等待时间
    
    // 专门查找与"unencrypted message"相关的输入框
    console.log('🔍 专门查找unencrypted message输入框...');
    
    // 尝试找到与"unencrypted message"相关的输入框
    let inputElement = null;
    
    // 首先尝试查找可能与unencrypted message相关的特定选择器
    const unencryptedSelectors = [
    ...
    for (const selector of unencryptedSelectors) {
      try {
        inputElement = await page.$(selector);
        if (inputElement) {
          // 检查这个元素是否与"unencrypted message"相关
          const textContent = await page.evaluate(el => el.textContent || '', inputElement);
          const placeholder = await page.evaluate(el => el.getAttribute('placeholder') || '', inputElement);
          const ariaLabel = await page.evaluate(el => el.getAttribute('aria-label') || '', inputElement);
          
          console.log(`检查元素 - 文本内容: "${textContent.substring(0, 50)}", 占位符: "${placeholder}", ARIA标签: "${ariaLabel}"`);
          
          // 如果元素包含与消息相关的文本或标签，使用它
          if (textContent.includes('message') || placeholder.includes('message') || 
              ariaLabel.includes('message') || ariaLabel.includes('Message') ||
              placeholder.includes('unencrypted') || textContent.includes('unencrypted')) {
            console.log(`✅ 找到与消息相关的输入框: ${selector}`);
            break;
          } else {
            console.log(`⚠️ 元素不匹配条件，继续查找...`);
            inputElement = null;
          }
        }
      } catch (e) {
    ...
    // 如果仍然没有找到，使用通用选择器查找textarea
    if (!inputElement) {
      console.log('⚠️ 未找到特定的unencrypted message输入框，使用通用选择器...');
      ...
      for (const selector of genericSelectors) {
        try {
          inputElement = await page.$(selector);
          if (inputElement) {
            console.log(`✅ 找到通用消息输入框: ${selector}`);
            break;
          }
        } catch (e) {
          continue;
    ...
    // 直接在unencrypted message输入框中输入消息
    try {
      // 检查元素类型并使用适当的方法
    ...
    console.log(`📤 消息已输入到unencrypted message输入框: "${message}"`);
...
// 运行测试
testUnencryptedInputDirect().then(results => {
  console.log('\n🏁 直接在unencrypted message输入框测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 直接在unencrypted message输入框测试脚本执行失败:', error);
  process.exit(1);
});