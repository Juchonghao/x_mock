...
/**
 * 修复版测试给特定用户发送私信 - 优化信封图标按钮查找和消息发送
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testEnvelopeDMButtonFixed() {
  console.log('🚀 开始修复版测试信封图标私信按钮');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `fixed-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        await new Promise(resolve => setTimeout(resolve, 3000));
        await page.screenshot({ 
          path: path.join(sessionDir, `fixed-dm-ready-${username}.png`), 
          fullPage: true 
        });
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `fixed-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'fixed-test-complete.png'), 
      fullPage: true 
    });
...
async function openDMFromUserProfileWithEnvelopeButton(page, username) {
  try {
    ...
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/fixed-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/fixed-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await new Promise(resolve => setTimeout(resolve, 5000));
    await page.screenshot({ 
      path: `./sessions/fixed-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
...
async function handlePinVerification(page) {
  try {
    ...
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/fixed-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/fixed-after-pin-submit.png', 
      fullPage: true 
    });
...
// 运行测试
testEnvelopeDMButtonFixed().then(results => {
  console.log('\n🏁 修复版测试脚本执行完毕');
  process.exit(0);
}).catch(error => {
  console.error('❌ 修复版测试脚本执行失败:', error);
  process.exit(1);
});