...
async function testCompleteFix() {
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'complete-fix-initial.png'), 
      fullPage: true 
    });
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        // 点击"New message"按钮
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChatComplete(page);
        ...
        const newChatPasscodeHandled = await handlePasscodeInNewChatComplete(page);
        ...
        const userFound = await searchUserInNewChatComplete(page, username);
        ...
        // 点击用户开始聊天 - 完整修复版本（包括资料页到私信流程）
        console.log(`💬 点击用户开始聊天...`);
        const chatStarted = await startChatWithUserComplete(page, username);
        ...
        const chatPasscodeHandled = await handlePasscodeInChatComplete(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `complete-fix-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessageComplete(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过完整修复版本的聊天流程发送的测试。祝你一切顺利！🤖`;
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `complete-fix-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'complete-fix-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 打开新聊天对话框 - 完整修复版本
 */
async function openNewChatComplete(page) {
  try {
    console.log('🔍 查找新消息按钮...');
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/complete-fix-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/complete-fix-after-new-chat.png', 
      fullPage: true 
    });
...
/**
 * 在新聊天界面处理passcode验证 - 完整修复版本
 */
async function handlePasscodeInNewChatComplete(page) {
  try {
 ...
 * 在新聊天界面中搜索用户 - 完整修复版本
 */
async function searchUserInNewChatComplete(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/complete-fix-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/complete-fix-search-${username}-after.png`, 
      fullPage: true 
    });
...
/**
 * 点击用户开始聊天 - 完整修复版本（包括资料页到私信流程）
 * 重点修复搜索结果中点击用户的功能，以及从资料页到私信的流程
 */
async function startChatWithUserComplete(page, username) {
  try {
    ...
    // 拍摄搜索结果截图，帮助调试
    await page.screenshot({ 
      path: `./sessions/complete-fix-search-results-${username}.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后截图
    await page.screenshot({ 
      path: `./sessions/complete-fix-after-click-${username}.png`, 
      fullPage: true 
    });
        ...
        try {
          ...
          if (dmButton) {
            ...
            try {
              ...
              console.log('⚠️ 等待私信按钮可点击时出错，继续尝试点击');
            }
            ...
            if (newUrl.includes('/messages/') || newUrl.includes('/chat/')) {
              ...
              // 如果仍然没有进入聊天界面，可能需要再次等待并检查
              await new Promise(resolve => setTimeout(resolve, 5000));
              const finalUrl = await page.url();
              if (finalUrl.includes('/messages/') || finalUrl.includes('/chat/')) {
                console.log(`✅ 最终成功进入与 @${username} 的聊天界面`);
                return true;
              }
            }
            break;
          ...
          console.log(`⚠️ 未找到私信按钮 ${selector}:`, e.message);
          continue;
        }
      }
      
      if (!dmButtonFound) {
        console.log(`❌ 在用户资料页未找到私信按钮`);
        return false;
      }
    }
    ...
    console.log(`最终聊天页面URL: ${currentUrl}`);
...
async function handlePinVerificationComplete(page) {
  try {
    ...
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/complete-fix-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/complete-fix-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 在聊天中处理passcode验证 - 完整修复版本
 */
async function handlePasscodeInChatComplete(page) {
  try {
...
/**
 * 检查是否有"unencrypted message"提示 - 完整修复版本
 */
async function checkUnencryptedMessageComplete(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
...
// 运行测试
testCompleteFix().then(results => {
  console.log('\n🏁 完整修复版本测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 完整修复版本测试脚本执行失败:', error);
  process.exit(1);
});