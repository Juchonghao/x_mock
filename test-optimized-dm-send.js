...
async function testOptimizedDMSend() {
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `optimized-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        const dmOpened = await openDMFromUserProfileOptimized(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationOptimized(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `optimized-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是优化版测试，确保私信能够成功发送。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageOptimized(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `optimized-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'optimized-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 从用户资料页面打开私信 - 优化版
 */
async function openDMFromUserProfileOptimized(page, username) {
  try {
    console.log(`🔍 在 @${username} 的页面上查找私信按钮...`);
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/optimized-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/optimized-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: `./sessions/optimized-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 优化版
 */
async function handlePinVerificationOptimized(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/optimized-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/optimized-after-pin-submit.png', 
      fullPage: true 
    });
 ...
 * 发送私信消息 - 优化版，确保消息真正发送
 */
async function sendDMMessageOptimized(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testOptimizedDMSend().then(results => {
  console.log('\n🏁 优化版测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 优化版测试脚本执行失败:', error);
  process.exit(1);
});