...
async function testNewChatFlow() {
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'new-chat-flow-initial.png'), 
      fullPage: true 
    });
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChat(page);
        ...
        const userFound = await searchUserInNewChat(page, username);
        ...
        const chatStarted = await startChatWithUser(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationNewChat(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `new-chat-flow-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有passcode验证
        console.log('🔍 检查是否需要passcode验证...');
        const passcodeHandled = await handlePasscodeIfPresentNewChat(page);
        ...
        const unencryptedMessageFound = await checkUnencryptedMessageNewChat(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过新聊天流程发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageNewChat(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `new-chat-flow-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'new-chat-flow-test-complete.png'), 
      fullPage: true 
    });
...
async function openNewChat(page) {
  try {
    ...
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/new-chat-flow-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/new-chat-flow-after-new-chat.png', 
      fullPage: true 
    });
    
    // 检查是否成功打开新聊天界面
    const currentUrl = await page.url();
    console.log(`新聊天页面URL: ${currentUrl}`);
    
    // 检查页面内容是否包含搜索框
    const pageContent = await page.content();
    if (pageContent.includes('search') || pageContent.includes('Search') || 
        pageContent.includes('搜索') || pageContent.includes('compose')) {
      console.log('✅ 成功打开新聊天界面');
      return true;
    }
    
    console.log('⚠️ 可能未成功打开新聊天界面，但仍继续');
    return true; // 即使URL没有变化，也继续
 ...
 * 在新聊天界面中搜索用户
 */
async function searchUserInNewChat(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/new-chat-flow-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/new-chat-flow-search-${username}-after.png`, 
      fullPage: true 
    });
 ...
 * 点击用户开始聊天
 */
async function startChatWithUser(page, username) {
  try {
    ...
    // 拍摄开始聊天后截图
    await page.screenshot({ 
      path: `./sessions/new-chat-flow-start-chat-${username}.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 新聊天流程版本
 */
async function handlePinVerificationNewChat(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/new-chat-flow-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/new-chat-flow-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 检查并处理passcode验证 - 新聊天流程版本
 */
async function handlePasscodeIfPresentNewChat(page) {
  try {
    console.log('🔍 检查是否需要passcode验证...');
...
/**
 * 检查是否有"unencrypted message"提示 - 新聊天流程版本
 */
async function checkUnencryptedMessageNewChat(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
 ...
 * 发送私信消息 - 新聊天流程版本
 */
async function sendDMMessageNewChat(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testNewChatFlow().then(results => {
  console.log('\n🏁 新聊天流程测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 新聊天流程测试脚本执行失败:', error);
  process.exit(1);
});