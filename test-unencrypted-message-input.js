...
/**
 * 专门针对unencrypted message输入框的私信发送测试
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testUnencryptedMessageInput() {
  console.log('🚀 开始unencrypted message输入框私信发送测试');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `unencrypted-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        const dmOpened = await openDMFromUserProfileUnencrypted(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationUnencrypted(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `unencrypted-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessageUnencrypted(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是专门针对unencrypted message输入框的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageUnencrypted(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `unencrypted-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'unencrypted-test-complete.png'), 
      fullPage: true 
    });
...
/**
 * 从用户资料页面打开私信 - unencrypted message版本
 */
async function openDMFromUserProfileUnencrypted(page, username) {
  try {
    console.log(`🔍 在 @${username} 的页面上查找私信按钮...`);
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/unencrypted-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/unencrypted-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: `./sessions/unencrypted-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
...
/**
 * 处理PIN验证 - unencrypted message版本
 */
async function handlePinVerificationUnencrypted(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/unencrypted-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/unencrypted-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 检查是否有"unencrypted message"提示 - unencrypted message版本
 */
async function checkUnencryptedMessageUnencrypted(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
...
/**
 * 发送私信消息 - unencrypted message版本，专门针对unencrypted message输入框
 */
async function sendDMMessageUnencrypted(page, message, username) {
  try {
    ...
    for (const selector of unencryptedSelectors) {
      try {
        ...
        if (inputElement) {
          console.log(`✅ 找到可能的unencrypted message输入框: ${selector}`);
          
          // 额外验证是否包含"unencrypted"相关文本
          ...
          if (textContent.includes('unencrypted') || placeholder.includes('unencrypted') || 
              textContent.includes('Unencrypted') || placeholder.includes('Unencrypted')) {
            console.log('✅ 确认找到unencrypted message输入框');
            break;
          } else {
            console.log(`⚠️ 输入框不包含unencrypted文本，继续查找...`);
            inputElement = null;
            continue;
          }
        }
    ...
    // 如果没有找到专门的unencrypted message输入框，使用通用选择器
    if (!inputElement) {
      console.log('⚠️ 未找到专门的unencrypted message输入框，使用通用选择器...');
...
// 运行测试
testUnencryptedMessageInput().then(results => {
  console.log('\n🏁 unencrypted message输入框测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ unencrypted message输入框测试脚本执行失败:', error);
  process.exit(1);
});