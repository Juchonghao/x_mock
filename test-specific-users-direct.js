...
/**
 * 直接使用Puppeteer测试给特定用户发送私信（不使用代理）
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testSpecificUsersDirect() {
  console.log('🚀 开始直接连接测试给特定用户发送私信');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `direct-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        const dmOpened = await openDMFromUserProfile(page, username);
        ...
        await new Promise(resolve => setTimeout(resolve, 3000));
        await page.screenshot({ 
          path: path.join(sessionDir, `direct-dm-ready-${username}.png`), 
          fullPage: true 
        });
        ...
        if (!isReady) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `direct-check-after-delay-${username}.png`), 
            fullPage: true 
          });
        }
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `direct-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'direct-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 从用户资料页面打开私信
 */
async function openDMFromUserProfile(page, username) {
  try {
    console.log(`🔍 在 @${username} 的页面上查找私信按钮...`);
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/direct-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/direct-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await new Promise(resolve => setTimeout(resolve, 5000));
    await page.screenshot({ 
      path: `./sessions/direct-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
...
async function handlePinVerification(page) {
  try {
    ...
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/direct-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/direct-after-pin-submit.png', 
      fullPage: true 
    });
...
// 运行测试
testSpecificUsersDirect().then(results => {
  console.log('\n🏁 直接连接版测试脚本执行完毕');
  process.exit(0);
}).catch(error => {
  console.error('❌ 直接连接版测试脚本执行失败:', error);
  process.exit(1);
});