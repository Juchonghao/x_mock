...
/**
 * 最终工作版本：修复所有问题的私信发送测试
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testFinalWorkingDM() {
  console.log('🚀 开始最终工作版本私信发送测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试修复了所有已知问题，包括selector错误和超时问题');
  ...
  try {
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `final-working-user-${username}-page.png`), 
          fullPage: true 
        });
        ...
        const dmOpened = await openDMFromUserProfileFinalWorking(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationFinalWorking(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `final-working-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有passcode验证
        console.log('🔍 检查是否需要passcode验证...');
        const passcodeHandled = await handlePasscodeIfPresentFinalWorking(page);
        ...
        const unencryptedMessageFound = await checkUnencryptedMessageFinalWorking(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是最终工作版本的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageFinalWorking(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `final-working-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'final-working-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 从用户资料页面打开私信 - 最终工作版本
 */
async function openDMFromUserProfileFinalWorking(page, username) {
  try {
    console.log(`🔍 在 @${username} 的页面上查找私信按钮...`);
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: `./sessions/final-working-open-dm-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: `./sessions/final-working-open-dm-${username}-after-click.png`, 
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: `./sessions/final-working-open-dm-${username}-waiting.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 最终工作版本
 */
async function handlePinVerificationFinalWorking(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/final-working-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/final-working-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 检查并处理passcode验证 - 最终工作版本
 */
async function handlePasscodeIfPresentFinalWorking(page) {
  try {
    console.log('🔍 检查是否需要passcode验证...');
...
/**
 * 检查是否有"unencrypted message"提示 - 最终工作版本
 */
async function checkUnencryptedMessageFinalWorking(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
...
/**
 * 发送私信消息 - 最终工作版本，修复selector错误
 */
async function sendDMMessageFinalWorking(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
    ...
    // 查找消息输入框 - 专门查找可能的unencrypted message输入框
    console.log('🔍 查找消息输入框...');
    
    let inputElement = null;
    
    // 尝试找到最可能的消息输入框，特别是与unencrypted message相关的
    ...
    // 确保输入元素是有效的DOM元素 - 修复selector错误的关键
    ...
    // 输入消息 - 完全修复selector错误
    try {
      // 直接使用page.evaluate来处理DOM元素，避免selector错误
...
// 运行测试
testFinalWorkingDM().then(results => {
  console.log('\n🏁 最终工作版本测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 最终工作版本测试脚本执行失败:', error);
  process.exit(1);
});