...
/**
 * 修复搜索功能和passcode验证的聊天流程测试
 * 重点修复搜索用户功能，同时保持passcode处理
 * 目标用户: kent236896, allen180929, fred_0201, Alex09936200
 */
async function testFixSearchAndPasscode() {
  console.log('🚀 开始修复搜索功能和passcode验证的聊天流程测试');
  console.log('🎯 目标用户: kent236896, allen180929, fred_0201, Alex09936200');
  console.log('💡 本次测试修复搜索功能，同时保持passcode处理');
  ...
  try {
    ...
    await page.screenshot({ 
      path: path.join(sessionDir, 'fix-search-passcode-initial.png'), 
      fullPage: true 
    });
    ...
    for (let i = 0; i < targetUsers.length; i++) {
      ...
      try {
        // 点击"New message"按钮
        console.log(`💬 点击新消息按钮...`);
        const newChatOpened = await openNewChatFixed(page);
        ...
        const newChatPasscodeHandled = await handlePasscodeInNewChatFixed(page);
        ...
        const userFound = await searchUserInNewChatFixed(page, username);
        ...
        const chatStarted = await startChatWithUserFixed(page, username);
        ...
        if (currentUrl.includes('/pin/recovery') || currentUrl.includes('/verify')) {
          ...
          // 处理PIN验证
          const pinSuccess = await handlePinVerificationFixed(page);
        ...
        const chatPasscodeHandled = await handlePasscodeInChatFixed(page);
        ...
        await page.screenshot({ 
          path: path.join(sessionDir, `fix-search-passcode-dm-ready-${username}.png`), 
          fullPage: true 
        });
        
        // 检查是否有"unencrypted message"提示
        console.log('🔍 检查是否有"unencrypted message"提示...');
        const unencryptedMessageFound = await checkUnencryptedMessageFixed(page);
        ...
        // 准备个性化消息
        const message = `你好 @${username}！这是一条来自X自动化机器人的测试私信。这是通过修复搜索和passcode验证的聊天流程发送的测试。祝你一切顺利！🤖`;
        
        console.log(`📝 发送私信内容: "${message}"`);
        const messageSent = await sendDMMessageFixed(page, message, username);
        ...
        if (messageSent) {
          ...
          await page.screenshot({ 
            path: path.join(sessionDir, `fix-search-passcode-message-sent-${username}.png`), 
            fullPage: true 
          });
    ...
    // 拍摄最终状态截图
    await page.screenshot({ 
      path: path.join(sessionDir, 'fix-search-passcode-test-complete.png'), 
      fullPage: true 
    });
 ...
 * 打开新聊天对话框 - 修复版本
 */
async function openNewChatFixed(page) {
  try {
    console.log('🔍 查找新消息按钮...');
    
    // 拍摄当前页面截图
    await page.screenshot({ 
      path: './sessions/fix-search-passcode-before-new-chat.png', 
      fullPage: true 
    });
    ...
    // 拍摄点击后的截图
    await page.screenshot({ 
      path: './sessions/fix-search-passcode-after-new-chat.png', 
      fullPage: true 
    });
...
/**
 * 在新聊天界面处理passcode验证 - 修复版本
 */
async function handlePasscodeInNewChatFixed(page) {
  try {
 ...
 * 重点修复selector错误
 */
async function searchUserInNewChatFixed(page, username) {
  try {
    console.log(`🔍 在新聊天界面中搜索用户 @${username}...`);
    
    // 拍摄搜索前截图
    await page.screenshot({ 
      path: `./sessions/fix-search-passcode-search-${username}-before.png`, 
      fullPage: true 
    });
    ...
    // 拍摄搜索后截图
    await page.screenshot({ 
      path: `./sessions/fix-search-passcode-search-${username}-after.png`, 
      fullPage: true 
    });
 ...
 * 点击用户开始聊天 - 修复版本
 */
async function startChatWithUserFixed(page, username) {
  try {
    ...
    if (!userElement) {
      ...
      // 查找所有可能的用户结果元素
      const possibleUserElements = await page.$$('.//div[contains(@role, "button") or contains(@role, "link")]');
    ...
    // 拍摄开始聊天后截图
    await page.screenshot({ 
      path: `./sessions/fix-search-passcode-start-chat-${username}.png`, 
      fullPage: true 
    });
 ...
 * 处理PIN验证 - 修复版本
 */
async function handlePinVerificationFixed(page) {
  try {
    console.log('🔐 处理PIN验证...');
    
    // 拍摄PIN验证页面截图
    await page.screenshot({ 
      path: './sessions/fix-search-passcode-pin-verification-page.png', 
      fullPage: true 
    });
    ...
    // 拍摄验证后截图
    await page.screenshot({ 
      path: './sessions/fix-search-passcode-after-pin-submit.png', 
      fullPage: true 
    });
...
/**
 * 在聊天中处理passcode验证 - 修复版本
 */
async function handlePasscodeInChatFixed(page) {
  try {
...
/**
 * 检查是否有"unencrypted message"提示 - 修复版本
 */
async function checkUnencryptedMessageFixed(page) {
  try {
    console.log('🔍 检查是否有"unencrypted message"提示...');
 ...
 * 发送私信消息 - 修复版本
 */
async function sendDMMessageFixed(page, message, username) {
  try {
    console.log('📝 准备发送消息...');
...
// 运行测试
testFixSearchAndPasscode().then(results => {
  console.log('\n🏁 修复搜索功能和passcode验证的聊天流程测试脚本执行完毕');
  console.log('💡 请检查目标用户的私信收件箱，消息应该已经发送成功');
  process.exit(0);
}).catch(error => {
  console.error('❌ 修复搜索功能和passcode验证的聊天流程测试脚本执行失败:', error);
  process.exit(1);
});